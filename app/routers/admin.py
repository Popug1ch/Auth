...
from app.schemas import UserWithRoles, RoleOut, RoleCreate, RoleUpdate, PermissionOut, PermissionCreate, AssignRole
...
async def create_role(
    ...
    if role_data.permission_ids:
        perms = await session.execute(select(Permission).where(Permission.id.in_(role_data.permission_ids)))
...
async def update_role(
    ...
    if role_data.name:
        existing = await session.execute(select(Role).where(Role.name == role_data.name, Role.id != role_id))
        if existing.scalars().first():
...
async def create_permission(
    ...
    existing = await session.execute(
        select(Permission).where(Permission.resource == perm_data.resource, Permission.action == perm_data.action)
    )
    if existing.scalars().first():
