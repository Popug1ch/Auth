...
def require_admin(session: SessionDep, current_user: User = Depends(get_current_active_user)):
...
def require_permission(resource: str, action: str):
    async def permission_checker(session: SessionDep, current_user: User = Depends(get_current_active_user)):
        for role in current_user.roles:
            if role.name == "admin":
                return current_user
